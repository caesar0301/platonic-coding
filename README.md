...
- **No Platonic infrastructure** (no `.platonic.yml` **and** no `docs/specs/`)? → Initialize (`init-scaffold`) or start the recovery flow (`init-scan` → recovery operations)
...
**Note**: `.platonic.yml` is optional. The skill auto-detects project metadata (name, language, framework) from host manifests (`Cargo.toml`, `package.json`, `pyproject.toml`, `go.mod`, etc.) and uses built-in default paths (`docs/specs`, `docs/impl`, `docs/drafts`). A `.platonic.yml` is only needed to override defaults or set custom spec stages.
...
```
Auto-detects missing infrastructure → scaffolds directories and templates. `.platonic.yml` is **not** created by default — it's only written when you request non-default paths or custom spec stages.
