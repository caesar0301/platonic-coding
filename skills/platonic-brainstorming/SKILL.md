...
description: "You MUST use this before any creative work in Platonic Coding - creating features, building components, adding functionality, or modifying behavior. Explores user intent, requirements, and design before implementation."
license: MIT
metadata:
  version: "1.0.0"
  author: "Xiaming Chen"
...
6. **Write design draft** - save to `docs/drafts/YYYY-MM-DD-<topic>-design.md` and commit
...
9. **Transition to the next Platonic stage** - route to internal substage `write impl guide`
...
digraph platonic_brainstorming {
    ...
    "Route to `write impl guide`" [shape=doublecircle];
    ...
    "User approves design?" -> "Write design draft" [label="yes"];
    ...
    "User reviews draft?" -> "Route to `write impl guide`" [label="approved"];
}
```

**The terminal state is `write impl guide`.** Do NOT invoke frontend-design, mcp-builder, or any other generic implementation skill. After Platonic Brainstorming, route into the Platonic Coding internal substage `write impl guide`.
...
- Commit the design draft to git
...
> "Design draft written and committed to `<path>`. Please review it and let me know if you want to make any changes before we advance to `write impl guide`."
...
- Route to the Platonic Coding internal substage `write impl guide`
- Do NOT invoke any other generic implementation skill. `write impl guide` is the next step.
